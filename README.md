...
```bash
cue export [<path>]           # 导出全部（含全文）为 JSON；默认存桌面 cue-clipboard-<时间戳>.json
...
- 相邻相同内容自动去重，仅更新时间
- 排序规则：置顶优先，其次按更新时间倒序
- 索引损坏自动恢复：`index.json` 无法解析时自动备份为 `index.json.bak`，从空库继续，不丢数据
- 单例保护：前台 `cue record` 与后台 daemon 通过 PID 文件互斥，避免重复入库
- 断更提醒：`cue status` 检测到超过 24 小时无新增记录时给出警告
